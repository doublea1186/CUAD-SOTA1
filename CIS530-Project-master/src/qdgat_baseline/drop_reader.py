...
YARD_NER_TYPE = "YARD"
DATE_PERIOD_RELATED_QS = ["effective_datetp#ckl", "expiration_datetp#ckl", "agreement_datetp#ckl", "renewal_termtp#ckl", "notice_period_to_terminate_renewaltp#ckl"]

# Agreement Date	Agreement Date-Answer	Effective Date	Effective Date-Answer	Expiration Date	Expiration Date-Answer	Renewal Term	Renewal Term-Answer	Notice Period To Terminate Renewal	Notice Period To Terminate Renewal- Answer
...
def retrieve_dates(passage_text):
    ...
    formatted_dates = [(date[2], date[3], date[4]) for date in dates]
    return ' '.join(raw_tokens), formatted_dates
...
class DropReader(object):
    ...
    def _read(self, file_path: str):
        ...
        for passage_id, passage_info in dataset.items():
            ...
            for question_answer in passage_info["qa_pairs"]:
                ...
                if not question_text in DATE_PERIOD_RELATED_QS:
                    continue
                answer_annotations = []
    ...
    def text_to_instance(self,
        ...
        passage_text, dates = retrieve_dates(passage_text)
        ...
        if answer_annotations:
            ...
            # TODO: wait for Danqi's new dataset here
            if answer_annotations[0]['date']['year'] == "20[]":
                print("here")
            answer_type, answer_raw_texts = self.extract_answer_info_from_annotation(answer_annotations[0])
            answer_texts = [" ".join(whitespace_tokenize(answer_text)) for answer_text in answer_raw_texts if not '' == answer_text]
        ...
        if self.instance_format == "drop":
            ...
            for node_id, gnode_type in enumerate(gnodes_type):
                gnode_id = gnodes[node_id]
                gnode_len = gnodes_len[node_id]
                gnode_token = passage_tokens[gnode_id: gnode_id + gnode_len]
                gnode_string = ' '.join(gnode_token).replace('Ġ', '')
                gnode_tokens.append(gnode_string)

                if gnode_type in ["DATE", "TIME"]:
                    formatted_date = process_date(gnode_string)
                    # _, formatted_date_2 = retrieve_dates(gnode_string)
                    # if not len(formatted_date_2) == 0:
                    #     print("here")
                    if formatted_date.is_date():
                        gnode_dates.append(formatted_date)
                    else:
                        gnode_dates.append(None)

                if gnode_type in ["PERIOD", "DURATION", "TIME"]:
                    formatted_period = process_period(gnode_string)
                    if formatted_period[0] is not None:
                        gnode_periods.append(formatted_period)
                    else:
                        gnode_periods.append(None)
            ...
            valid_signs_for_date_add_sub_expressions: List[List[int]] = []
            valid_signs_for_period: List[List[int]] = []
            ...
            if answer_type in ["number"]:
                ...
                valid_signs_for_date_add_sub_expressions = self.find_valid_date_add_sub_expressions(passage_tokens,
                gnode_dates, gnode_periods, gnodes_len, gnodes_type, answer_raw_texts)
                if len(valid_signs_for_date_add_sub_expressions) == 0:
                    return None
            elif answer_type in ["period"]:
                valid_signs_for_period = self.find_valid_period_expressions(passage_tokens,
                gnode_periods, gnodes_len, gnodes_type, ' '.join(answer_raw_texts))
                if len(valid_signs_for_period) == 0:
                    return None
            ...
            type_to_answer_map = {"passage_span": valid_passage_spans, "question_span": valid_question_spans,
                                  ...
                                  "multi_span": multi_span, "date": valid_signs_for_date_add_sub_expressions,
                                  "period": valid_signs_for_period}
            ...
            return self.make_marginal_drop_instance(question_tokens,
                                                    ...
                                                    additional_metadata={"original_passage": passage_text,
                                                                         ...
                                                                         "gnode_dates": gnode_dates,
                                                                         "gnode_periods": gnode_periods,
    ...
    def extract_answer_info_from_annotation(answer_annotation: Dict[str, Any]) -> Tuple[str, List[str]]:
        ...
        if answer_annotation["spans"]:
            ...
            answer_type = "date"
        elif any(answer_annotation["period"].values()):
            answer_type = "period"
        ...
        if answer_type is None:  # No answer
            ...
            date_tokens = [answer_content[key] for key in ["month", "day", "year"] if
                           key in answer_content]
            answer_texts = date_tokens
        elif answer_type == "period":
            period_tokens = [str(answer_content[key]) for key in ["time", "unit"] if
                             key in answer_content]
            answer_texts = period_tokens
    ...
    # TODO: Update date related calculations
    @staticmethod
    def find_valid_date_add_sub_expressions(
        passage_tokens, gnodes_dates, gnodes_periods, gnodes_len, gnodes_type, target_date, max_number_of_numbers_to_consider: int = 2) -> \
            List[List[int]]:

            valid_dates = []
            valid_periods = []
            valid_signs_for_date_add_sub_expressions = []
            assert len(target_date) == 3
            target_date = process_formatted_date(target_date[2], target_date[0], target_date[1])


            for date_id, formatted_date in enumerate(gnodes_dates):
                if not formatted_date is None:
                    if is_same_processed_date(formatted_date, target_date):
                        labels_for_gnodes = [0] * len(gnodes_dates)
                        labels_for_gnodes[date_id] = 1
                        valid_signs_for_date_add_sub_expressions.append(labels_for_gnodes)
                    valid_dates.append((date_id, formatted_date))

            for pid, period in enumerate(gnodes_periods):
                if period is not None:
                    time, unit = period
                    if time is None or unit is None:
                        continue
                    valid_periods.append((pid, time, unit))

            for (date_id, valid_date), (period_id, time, unit) in itertools.product(valid_dates, valid_periods):
                m, d, y = valid_date.to_num_tuple()
                default_month = '01'
                default_day = '01'
                default_year = '2020'
                ...
                if unit == "day":
                    ...
                    print("period should not have other unit")
                    continue

                result_date_add = iso_date + date_difference
                result_date = process_formatted_date(result_date_add.year, result_date_add.month, result_date_add.day)
                if is_same_processed_date(target_date, result_date):
                    labels_for_gnodes = [0] * len(gnodes_periods)
                    labels_for_gnodes[date_id] = 1
                    labels_for_gnodes[period_id] = 1
                    valid_signs_for_date_add_sub_expressions.append(labels_for_gnodes)

                result_date_minus = iso_date - date_difference
                result_date = process_formatted_date(result_date_minus.year, result_date_minus.month, result_date_minus.day)
                if is_same_processed_date(target_date, result_date):
                    labels_for_gnodes = [0] * len(gnodes_periods)
                    labels_for_gnodes[date_id] = 1
                    labels_for_gnodes[period_id] = 2
                    valid_signs_for_date_add_sub_expressions.append(labels_for_gnodes)

            return valid_signs_for_date_add_sub_expressions

    @staticmethod
    def find_valid_period_expressions(
        passage_tokens, gnode_periods, gnodes_len, gnodes_type, target_period, max_number_of_numbers_to_consider: int = 2) -> \
            List[List[int]]:

            valid_periods = []
            valid_signs_for_period_add_sub_expressions = []
            target_period = process_period(target_period)

            for pid, pred_period in enumerate(gnode_periods):
                if pred_period is not None:
                    time, unit = pred_period
                    if time is None or unit is None:
                        continue

                    if is_same_processed_period(target_period, pred_period):
                        labels_for_gnodes = [0] * len(gnode_periods)
                        labels_for_gnodes[pid] = 1
                        valid_signs_for_period_add_sub_expressions.append(labels_for_gnodes)

            return valid_signs_for_period_add_sub_expressions
