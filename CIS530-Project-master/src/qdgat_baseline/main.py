...
def preprocess_drop(args, tokenizer):
    for mode in ['train', 'dev']:
        ...
        data_fpath = os.path.join(args.data_dir, "cuad_dataset_number_%s_parsed.json" % mode)
        if not os.path.exists(data_fpath):
            raise Exception("Missing %s for preprocessing." % data_fpath)

        skip_when_all_empty = ["passage_span", "question_span",
                               "multi_span"] if mode == 'train' else None
        reader = DropReader(
        ...
        for batch in tqdm(train_itr):
            ...
            if update_cnt % (args.log_per_updates * args.gradient_accumulation_steps) == 0 or update_cnt == 1:
                logger.info("QDGAT train: step:{0:6} loss:{1:.5f} f1:{2:.5f} em:{3:.5f} left:{4}".format(
                    ...
                    str((datetime.now() - train_start) / (update_cnt + 1) * (num_train_steps - update_cnt - 1)).split(
                        '.')[0]))
                reset_metrics()

        if args.do_eval:
            eval_loss, eval_f1, eval_em, predictions = evaluate(args, network, dev_itr)
...
def evaluate(args, network, dev_itr):
    ...
    predictions = network.get_prediction()

    logger.info("Eval {} examples cost {}s.".format(example_cnt, (datetime.now() - eval_start).seconds))

    return loss_metric.avg, eval_metrics["f1"], eval_metrics["em"], predictions
...
def main():
    qdgat_data_dir = os.path.abspath(os.path.join(os.path.abspath(__file__), "../../../data/QDGAT"))
    parser = argparse.ArgumentParser()

    parser.add_argument("--passage_length_limit", type=int, default=10000)
    parser.add_argument("--question_length_limit", type=int, default=46)
    parser.add_argument("--data_dir", default=qdgat_data_dir, type=str, help="The data directory.")
    parser.add_argument("--save_dir", default="output", type=str, help=",The directory to save checkpoint.")
    ...
    parser.add_argument('--seed', type=int, default=1117, help='random seed for data shuffling, embedding init, etc.')
    ...
    parser.add_argument("--roberta_model", default="roberta-base", type=str, help="robert modle path.")
    ...
    if args.do_train:
        ...
        train_dataset = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.batch_size, num_workers=0,
                                   collate_fn=collate_fn, pin_memory=False)

    if args.do_eval:
        eval_dataset = DropBatchGen(args, data_mode="dev", tokenizer=tokenizer)
        eval_dataset = DataLoader(eval_dataset, batch_size=args.eval_batch_size, num_workers=0, collate_fn=collate_fn,
    ...
    if args.do_train:
        ...
        if args.pre_path is not None:
            ...
            optimizer = AdamW(optimizer_grouped_parameters, lr=args.learning_rate)
            load(network, optimizer, args.pre_path)
        eval_loss, eval_f1, eval_em, predictions = evaluate(args, network, eval_dataset)

        with open('./output/predictions.csv', mode='w', encoding='utf-8') as f:
        ...
        logger.info("loss {}, f1 {}, em {}.".format(eval_loss, eval_f1, eval_em))
