...
def get_metrics(predicted: Union[str, List[str], Tuple[str, ...]],
    ...
    if gold_type in ["span", "spans", "number"]:
        ...
        if predicted is None:
            ...
            # predicted_date = process_date(predicted)
            if is_same_processed_date(predicted, gold_answer):
                exact_match = 1.0
                f1 = 1.0
            else:
                exact_match = 0.0
    ...
    elif gold_type == "period":
        if predicted is None:
            exact_match = 0.0
            f1 = 0.0
        else:
            gold_answer = process_period(gold_answer)
            if is_same_processed_period(predicted, gold_answer):
                exact_match = 1.0
                f1 = 1.0
            else:
                exact_match = 0.0
...
def answer_json_to_strings(answer: Dict[str, Any]) -> Tuple[Tuple[str, ...], str]:
    ...
    elif "period" in answer and answer["period"]:
        return tuple([f"{answer['period']['time']} {answer['period']['unit']}"]), "period"
    else:
        raise ValueError(f"Answer type not found, should be one of number, spans or date at: {json.dumps(answer)}")

def answer_json_to_structured_data(answer: Dict[str, Any]) -> Tuple[Tuple[str, ...], str]:
    """
    Takes an answer JSON blob from the DROP data release and converts it into strings used for
    evaluation.
    """
    if "number" in answer and answer["number"]:
        ...
        return tuple(answer["spans"]), "span" if len(answer["spans"]) == 1 else "spans"
    elif "date" in answer and (answer["date"]["year"] != '' or answer["date"]["month"] != '' or answer["date"]["day"] != ''):


        date_string = f"{answer['date']['year']}, {answer['date']['month']}, {answer['date']['day']}"
        return process_date(date_string), "date"

    elif "period" in answer and (answer['period']['time'] != '' and answer['period']['time'] != ''):
        return f"{answer['period']['time']} {answer['period']['unit']}", "period"
    else:
        raise ValueError(f"Answer type not found, should be one of number, spans or date at: {json.dumps(answer)}")
        ...
        for qa_pair in annotation["qa_pairs"]:
            ...
            if query_id in predicted_answers:
                ...
                for answer in candidate_answers:
                    ...
                    gold_answer, gold_type = answer_json_to_structured_data(answer)
                ...
                if qa_pair and qa_pair["answer"]:
                    ...
                    max_type = answer_json_to_structured_data(qa_pair["answer"])[1]
                else:
                    max_type = "number"
                max_em_score = 0.0
                max_f1_score = 0.0
