...
import os
import sys
commonpath = os.path.abspath(os.path.join(os.path.abspath(__file__), "../../"))
sys.path.append(commonpath)
from comparison import is_same_processed_date, is_same_processed_period
from number_processing import process_date, process_period

def process_formatted_date(year, month, day):
    formatted_date = f"{month} {day}, {year}"
    return process_date(formatted_date)
...
class QDGATNet(nn.Module):
    ...
    def __init__(self,
        ...
        if answering_abilities is None:
            self.answering_abilities = ["date_calculation", "period_calculation"]
        else:
        ...
        if "date_calculation" in self.answering_abilities:
            self._date_calculation_index = self.answering_abilities.index("date_calculation")
            self._date_sign_predictor = BertFeedForward(4 * hidden_size, hidden_size, 3)

        if "period_calculation" in self.answering_abilities:
            self._period_calculation_index = self.answering_abilities.index("period_calculation")
            # Not add / subtract required for period calculation
            self._period_sign_predictor = BertFeedForward(4 * hidden_size, hidden_size, 2)
...
#        self.gat = GAT(node_dim, node_dim, dropout=0.6, alpha=0.2, nheads=2, edge_type_num=9)
    ...
    def forward(self,  # type: ignore
                ...
                answer_as_dates: torch.LongTensor = None,
                answer_as_period: torch.LongTensor = None,
        ...
        if "date_calculation" in self.answering_abilities:
            alg_encoded_numbers = torch.cat(
                [new_gnodes_embs,
                 question_h2.unsqueeze(1).repeat(1, new_gnodes_embs.size(1), 1),
                 passage_h2.unsqueeze(1).repeat(1, new_gnodes_embs.size(1), 1),
                 sequence_output[:, 0].unsqueeze(1).repeat(1, new_gnodes_embs.size(1), 1)
                 ], 2)

            # Shape: (batch_size, # of numbers in the passage, 3)
            date_sign_logits = self._date_sign_predictor(alg_encoded_numbers)
            date_sign_log_probs = torch.nn.functional.log_softmax(date_sign_logits, -1)

            # Shape: (batch_size, # of numbers in passage).
            best_signs_for_dates = torch.argmax(date_sign_log_probs, -1)
            # For padding numbers, the best sign masked as 0 (not included).
            # best_signs_for_numbers = util.replace_masked_values(best_signs_for_numbers, number_mask, 0)
            # Shape: (batch_size, # of numbers in passage)
            best_signs_date_log_probs = torch.gather(date_sign_log_probs, 2, best_signs_for_dates.unsqueeze(-1)).squeeze(
                -1)
            # the probs of the masked positions should be 1 so that it will not affect the joint probability
            # TODO: this is not quite right, since if there are many numbers in the passage,
            # TODO: the joint probability would be very small.
            # best_signs_log_probs = util.replace_masked_values(best_signs_log_probs, number_mask, 0)
            # Shape: (batch_size,)
            best_date_combination_log_prob = best_signs_date_log_probs.sum(-1)
            if len(self.answering_abilities) > 1:
                best_date_combination_log_prob += answer_ability_log_probs[:, self._date_calculation_index]
        ...
        if "period_calculation" in self.answering_abilities:
            ...
            # Shape: (batch_size, # of numbers in the passage, 3)
            period_sign_logits = self._period_sign_predictor(alg_encoded_numbers)
            period_sign_log_probs = torch.nn.functional.log_softmax(period_sign_logits, -1)

            # Shape: (batch_size, # of numbers in passage).
            best_signs_for_periods = torch.argmax(period_sign_log_probs, -1)
            # For padding numbers, the best sign masked as 0 (not included).
            # best_signs_for_numbers = util.replace_masked_values(best_signs_for_numbers, number_mask, 0)
            # Shape: (batch_size, # of numbers in passage)
            best_period_signs_log_probs = torch.gather(period_sign_log_probs, 2, best_signs_for_periods.unsqueeze(-1)).squeeze(
                -1)
            # the probs of the masked positions should be 1 so that it will not affect the joint probability
            # TODO: this is not quite right, since if there are many numbers in the passage,
            # TODO: the joint probability would be very small.
            # best_signs_log_probs = util.replace_masked_values(best_signs_log_probs, number_mask, 0)
            # Shape: (batch_size,)
            best_period_combination_log_prob = best_period_signs_log_probs.sum(-1)
            if len(self.answering_abilities) > 1:
                best_period_combination_log_prob += answer_ability_log_probs[:, self._period_calculation_index]
        ...
        if answer_as_passage_spans is not None or answer_as_question_spans is not None or answer_as_add_sub_expressions is not None or answer_as_counts is not None or answer_as_period is not None  or answer_as_dates is not None:
...
#                    print('log_marginal_likelihood_for_add_sub: ', log_marginal_likelihood_for_add_sub)
                ...
                elif answering_ability == "date_calculation":
                    # The padded add-sub combinations use 0 as the signs for all numbers, and we mask them here.
                    # Shape: (batch_size, # of combinations)
                    gold_add_date_mask = (answer_as_dates.sum(-1) > 0).float()
                    # Shape: (batch_size, # of numbers in the passage, # of combinations)
                    gold_add_date_signs = answer_as_dates.transpose(1, 2)
                    # Shape: (batch_size, # of numbers in the passage, # of combinations)
                    log_likelihood_for_date_signs = torch.gather(date_sign_log_probs, 2, gold_add_date_signs)
                    # the log likelihood of the masked positions should be 0
                    # so that it will not affect the joint probability
                    # log_likelihood_for_date_signs = util.replace_masked_values(log_likelihood_for_date_signs,
                                                                                #  gnodes_mask.unsqueeze(-1), 0)
                    # Shape: (batch_size, # of combinations)
                    log_likelihood_for_date = log_likelihood_for_date_signs.sum(1)
                    # For those padded combinations, we set their log probabilities to be very small negative value
                    log_likelihood_for_date = util.replace_masked_values(log_likelihood_for_date,
                                                                             gold_add_date_mask, -1e7)
                    # Shape: (batch_size, )
                    log_marginal_likelihood_for_date = util.logsumexp(log_likelihood_for_date)
                    log_marginal_likelihood_list.append(log_marginal_likelihood_for_date)
#                    print('log_marginal_likelihood_for_add_sub: ', log_marginal_likelihood_for_add_sub)

                elif answering_ability == "period_calculation":
                    # The padded add-sub combinations use 0 as the signs for all numbers, and we mask them here.
                    # Shape: (batch_size, # of combinations)
                    gold_add_period_mask = (answer_as_period.sum(-1) > 0).float()
                    # Shape: (batch_size, # of numbers in the passage, # of combinations)
                    gold_add_period_signs = answer_as_period.transpose(1, 2)
                    # Shape: (batch_size, # of numbers in the passage, # of combinations)
                    log_likelihood_for_period_signs = torch.gather(period_sign_log_probs, 2, gold_add_period_signs)
                    # the log likelihood of the masked positions should be 0
                    # so that it will not affect the joint probability
                    # TODO: record masks for period and dates
                    # log_likelihood_for_period_signs = util.replace_masked_values(log_likelihood_for_period_signs,
                                                                                #  gnodes_mask.unsqueeze(-1), 0)
                    # Shape: (batch_size, # of combinations)
                    log_likelihood_for_period = log_likelihood_for_period_signs.sum(1)
                    # For those padded combinations, we set their log probabilities to be very small negative value
                    log_likelihood_for_period = util.replace_masked_values(log_likelihood_for_period,
                                                                             gold_add_period_mask, -1e7)
                    # Shape: (batch_size, )
                    log_marginal_likelihood_for_period = util.logsumexp(log_likelihood_for_period)
                    log_marginal_likelihood_list.append(log_marginal_likelihood_for_period)
#                    print('log_marginal_likelihood_for_add_sub: ', log_marginal_likelihood_for_add_sub)
...
#            print('batch_loss: ', output_dict["loss"])
        ...
        with torch.no_grad():
            ...
            if metadata is not None:
                ...
                while i < batch_size:
                    ...
                    # elif predicted_ability_str == "date_calculation":  # plus_minus combination answer
                    #     answer_json["answer_type"] = "date"
                    #     original_numbers = metadata[i]['gnode_tokens']
                    #     sign_remap = {0: 0, 1: 1, 2: -1}
                    #     predicted_signs = [sign_remap[it] for it in best_signs_for_numbers[i].detach().cpu().numpy()]
                    ...
                    #     answer_json['number_sign_log_probs'] = number_sign_log_probs[i, :, :].detach().cpu().numpy()
                        ...
                        answer_json["answer_type"] = "date"
                        original_dates = metadata[i]['gnode_dates']
                        original_periods = metadata[i]['gnode_periods']

                        sign_remap = {0: 0, 1: 1, 2: -1}
                        predicted_signs = [sign_remap[it] for it in best_signs_for_dates[i].detach().cpu().numpy()]
                        predicted_signs_ct = sum([1 if sign_remap[it] != 0 else 0 for it in best_signs_for_dates[i].detach().cpu().numpy()])
                        if predicted_signs_ct > 2 or predicted_signs_ct == 0:
                        ...
                        answer_json['dates'] = []
                        answer_json['periods'] = []
                        answer_json['operations'] = []

                        for predicted_sign, original_date, original_period in zip(predicted_signs, original_dates, original_periods):
                            if predicted_sign == 1:
                                if original_date is not None:
                                    date = original_date
                                    answer_json['dates'].append(date)
                                if original_period is not None:
                                    period = original_period
                                    answer_json['periods'].append(period)
                                    operation = "plus"
                                    answer_json['operations'].append(operation)
                        ...
                        if period is not None:
                            ...
                            iso_date_string = f"{y}-{m}-{d}"
                            iso_date = datetime.date.fromisoformat(iso_date_string)

                            if unit == "day":
                                date_difference = relativedelta(days=time)
                            elif unit == "month":
                                date_difference = relativedelta(months=time)
                            elif unit == "year":
                                date_difference = relativedelta(years=time)


                            if operation == "plus":
                                result = iso_date + date_difference
                                result = process_formatted_date(result.year, result.month,
                                                                     result.day)
                        ...
                        answer_json['date_sign_log_probs'] = date_sign_log_probs[i, :, :].detach().cpu().numpy()

                    elif predicted_ability_str == "period_calculation":  # plus_minus combination answer
                        answer_json["answer_type"] = "period"
                        original_periods = metadata[i]['gnode_periods']

                        sign_remap = {0: 0, 1: 1}
                        predicted_signs = [sign_remap[it] for it in best_signs_for_periods[i].detach().cpu().numpy()]
                        predicted_signs_ct = sum([1 if sign_remap[it] != 0 else 0 for it in best_signs_for_periods[i].detach().cpu().numpy()])
                        if predicted_signs_ct > 1 or predicted_signs_ct == 0:
                            print("incorrect answer")
                            result = None
                        ...
                        for predicted_sign, original_period in zip(predicted_signs, original_periods):
                            if predicted_sign == 1:
                                if original_period is not None:
                                    period = original_period
                                    if not type(period) == tuple:
                                        period = None
                                    result = period
                                    answer_json['periods'].append(period)

                        answer_json["value"] = period
                        predicted_answer = period
                        answer_json['period_sign_log_probs'] = period_sign_log_probs[i, :, :].detach().cpu().numpy()
                    ...
                    if answer_annotations:
                        self._drop_metrics(predicted_answer, answer_annotations, question_id)
    ...
    def get_prediction(self):
        return self._drop_metrics.get_prediction()
