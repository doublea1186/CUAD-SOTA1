...
from number_processing import process_date, PartialDate, process_period
...
def create_collate_fn(padding_idx=1, use_cuda=False):
    def collate_fn(batch):
        ...
        for ct, v in enumerate(batch):
            v['answer_as_dates'][v['answer_as_dates'] == -1] = 0
            v['answer_as_period'][v['answer_as_period'] == -1] = 0
            if len(v['answer_as_dates'].shape) == 1:
                batch[ct]['answer_as_dates'] = v['answer_as_dates'].reshape(1, v['answer_as_dates'].shape[0])
            if len(v['answer_as_period'].shape) == 1:
                batch[ct]['answer_as_period'] = v['answer_as_period'].reshape(1, v['answer_as_period'].shape[0])

        max_dates_choice = max([v['answer_as_dates'].shape[0] for v in batch])
        answer_as_dates = torch.LongTensor(bsz, max_dates_choice, max_gnodes_len_choice).fill_(0)
        max_period_choice = max([v['answer_as_period'].shape[0] for v in batch])
        answer_as_period = torch.LongTensor(bsz, max_period_choice, max_gnodes_len_choice).fill_(0)
        ...
        for i in range(bsz):
            ...
            try:
                ...
                add_sub_len, num_len = item['answer_as_period'].shape
                answer_as_period[i][:add_sub_len,:num_len] = torch.LongTensor(item['answer_as_period'])
                add_sub_len, num_len = item['answer_as_dates'].shape
                answer_as_dates[i][:add_sub_len,:num_len] = torch.LongTensor(item['answer_as_dates'])
        ...
        out_batch = {"input_ids": input_ids, "input_mask": input_mask, "input_segments": input_segments,
               ...
               "answer_as_dates": answer_as_dates,
               "answer_as_period": answer_as_period,
...
class DropDataBuilder(object):
    ...
    def build(self, batch):
        ...
        answer_as_dates = np.full([gnode_cnt], 0)
        answer_as_period = np.full([gnode_cnt], 0)
        ...
        if len(metas['answer_info']['date']) > 0:
            answer_as_dates = np.array(metas['answer_info']['date'])

        if len(metas['answer_info']['period']) > 0 :
            answer_as_period = np.array(metas['answer_info']['period'])

        if np.sum(answer_as_dates) == 0 and np.sum(answer_as_period) == 0:
            return None
        ...
        out_batch = {"input_ids": input_ids, "input_mask": input_mask, "input_segments": input_segments,
                     ...
                     "gnodes_len": gnodes_len,
                     "gnodes_mask": gnodes_mask,
                     "gnodes_tokens": metas["gnode_tokens"],
                     "gnodes_dates": metas["gnode_dates"],
                     "gnodes_periods": metas["gnode_periods"],
                     "gedges": gedges,
                     "gedge_types": batch_gedge_types,
                     "answer_as_passage_spans": answer_as_passage_spans,
                     "answer_as_question_spans": answer_as_question_spans,
